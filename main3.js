...
var CLIENT_ID = '1031694725285-jsd9brm9i29bri0bpjdu0r3ck3751kqo.apps.googleusercontent.com';  // ضع هنا Client ID الخاص بك
var CLIENT_SECRET = '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';  // ضع هنا Client Secret الخاص بك
...
function fetchNewsWithImages() {
    ...
    try {
      ...
      // معالجة كل منشور
      items.slice(0, 5).forEach(function(item) { // جلب آخر 5 منشورات فقط
        ...
        var imageUrl = "";
        var contentElement = item.getChild("content:encoded") || item.getChild("description");
        if (contentElement) {
        ...
        var postContent = `
          <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto;">
            ...
            ${imageUrl ? `<img src="${imageUrl}" style="max-width: 100%; height: auto; margin: 15px 0; border-radius: 5px;" alt="${title}">` : ''}
            ...
            <div style="margin-top: 20px; font-style: italic; color: #666;">
              ...
              <p>تاريخ النشر: ${new Date(pubDate).toLocaleDateString('ar-EG')}</p>
            </div>
          </div>
        `;
  ...
  function createBloggerPost(blogId, apiKey, title, content) {
    ...
    var headers = {
      "Authorization": "Bearer " + Utilities.base64Encode(CLIENT_ID + ':' + CLIENT_SECRET),
